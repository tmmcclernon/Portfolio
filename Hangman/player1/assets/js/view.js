...
async function addDataToTable(url){
    ...
    //console.log(data)
    //clear table for refreshes
    

    //populate rows
   for(let i =0 ; i<data.length;i++){ //parse thru number of entries in database
    newRow = document.createElement("tr")
    usernameData = document.createTextNode(data[i].username)//append username to table
    newRow.appendChild(usernameData)
    newData = document.createElement("td")
    scoreData = document.createTextNode(data[i].score)//append score
    newData.appendChild(scoreData)
    newRow.appendChild(newData)
    highscoreTable.appendChild(newRow)
   }
}
async function addStatsToTable(url){
    highscoreTable = document.getElementById("statsTable")
